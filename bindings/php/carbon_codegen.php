...
if (!function_exists('carbon_schema_models')) {
    ...
    function carbon_schema_models($schema = null, ?string $namespace = null): string
    {
        ...
        foreach (($metadata['tables'] ?? []) as $table) {
            ...
            $lines[] = '    }';
            $lines[] = '';
            $lines[] = '    public static function Get(array $query = [], $schema = null, string $dialect = \\CarbonDialect::MYSQL): array';
            $lines[] = '    {';
            $lines[] = '        return \\carbon_model_get_request(self::class, $query, $schema, $dialect);';
            $lines[] = '    }';
            if ($columns !== []) {
                $lines[] = '';
